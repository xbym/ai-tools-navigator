...
export default function AIToolList({ tools }: AIToolListProps) {
  console.log('Rendering AIToolList, tools:', tools);
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {tools.map((tool) => (
        <Link href={`/tool/${tool._id}`} key={tool._id} className="flex">
          <div className="bg-gray-800 rounded-lg shadow-lg p-4 hover:bg-gray-700 transition duration-300 flex flex-col justify-between w-full">
            <div className="flex items-center mb-3">
              ...
              />
              <h2 className="text-lg font-semibold text-blue-400">{tool.name}</h2>
            </div>
