...
- 实现了用户登录功能，包括登录 API 和登录页面
- 集成了 JWT 用于用户认证
- 更新了项目依赖，添加了 jsonwebtoken
- 创建了 ErrorBoundary 组件和 Logger 类用于错误处理和日志记录
- 集成了 Sentry 用于错误监控
- 优化了项目结构，添加了全局布局组件
- 配置了站点地图生成
- 实现了用户登录功能，包括登录 API 和登录页面
...
- 创建了 ErrorBoundary 组件和 Logger 类用于错误处理和日志记录
- 进一步优化了 Sentry 集成
- 更新了项目配置，包括 Next.js 配置和 TypeScript 配置
- 优化了项目结构，添加了全局布局组件和响应式导航栏
...
## 注意事项
- 确保已生成并安全存储 JWT_SECRET 环境变量
- 不同环境使用不同的 JWT 密钥，并定期更换
- 对于没有 OpenSSL 的系统，使用 Node.js 的 crypto 模块生成随机字符串作为 JWT 密钥
